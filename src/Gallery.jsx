...
const Gallery = () => {
  ...
  // Handler to enhance image using DeepAI API
  const handleEnhanceImage = async (src) => {
    ...
    try {
      ...
      if (result.data && result.data.output_url) {
        // For session-based enhancement, you could uncomment the next line
  ...
  return (
    <>
      ...
      <div id="gallery-root">
        ...
        {enhanceError && <div style={{ color: 'red', textAlign: 'center', margin: 12 }}>{enhanceError}</div>}
        ...
        {selectedImg && (
          <div className="modal-overlay" onClick={() => setSelectedImg(null)}>
            <div className="modal-content" onClick={e => e.stopPropagation()}>
              ...
              <button
                style={{ marginTop: 16, width: '100%', background: '#0a2342', color: '#fff', border: 'none', borderRadius: 6, padding: '10px 0', cursor: isEnhancing ? 'not-allowed' : 'pointer', fontWeight: 500, fontSize: 18 }}
                disabled={isEnhancing}
